package fr.mrpiano.hardium.render;
...
public class WCCuboidNSEWUDRenderer implements ISimpleBlockRenderingHandler {
    ...
    public void renderInventoryBlock(Block block, int meta, int modelID,
        ...
        List<HardiumBlockDef.Cuboid> cubs = cblock.getCuboidList(meta);
        if (cubs != null) {
            int cnt = cubs.size();
            for (int i = 0; i < cnt; i++) {
                HardiumBlockDef.Cuboid cub = cubs.get(i);
                ...
                if (HardiumBlockDef.SHAPE_CROSSED.equals(cub.shape)) {
    ...
    private double calcInterpUV(IIcon ico, CalcUV f, HardiumBlockDef.Cuboid cub, boolean get_v, boolean min) {
        double v = 0.0;
        switch (f) {
            case XMIN:
                if (min)
    ...
    private double calcMinU(IIcon ico, int side, int rot, HardiumBlockDef.Cuboid cub) {
        return calcInterpUV(ico, uvCalc[side][rot][0], cub, false, true);
    }
    private double calcMaxU(IIcon ico, int side, int rot, HardiumBlockDef.Cuboid cub) {
        return calcInterpUV(ico, uvCalc[side][rot][0], cub, false, false);
    }
    private double calcMinV(IIcon ico, int side, int rot, HardiumBlockDef.Cuboid cub) {
        return calcInterpUV(ico, uvCalc[side][rot][1], cub, true, true);
    }
    private double calcMaxV(IIcon ico, int side, int rot, HardiumBlockDef.Cuboid cub) {
    ...
    private void renderCuboidSide(WCCuboidBlock block, HardiumBlockDef.Cuboid cub, RenderBlocks renderer, int meta, int side) {
        Tessellator tessellator = Tessellator.instance;
    ...
    private void renderCuboid(WCCuboidBlock block, HardiumBlockDef.Cuboid cub, RenderBlocks renderer, int meta, int x, int y, int z) {
        // Get absolute coordinates of corners
        xr[0] = x + cub.xMin;
        xr[1] = x + cub.xMax;
        yr[0] = y + cub.yMin;
