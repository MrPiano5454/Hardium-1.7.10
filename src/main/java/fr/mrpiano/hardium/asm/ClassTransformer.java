...
public class ClassTransformer implements IClassTransformer, Opcodes {
    ...
    private byte[] transformWorldType(String name, byte[] b, boolean obfus) {
        ...
        Hardium.log.fine("Checking class " + name);
        ...
        if (m == null) {
            Hardium.log.warning("Cannot find "  + targetMethodName + "() in " + name + " for patching");
            return b;
        }
        // Find our target op sequence
        int ldx_index = findOpSequence(m, new int[] { LDC } );
        if (ldx_index < 0) {
            Hardium.log.warning("Cannot patch "  + targetMethodName + "() in " + name);
            return b;
        }
        ...
        Hardium.log.fine("Method " + targetMethodName + "() of " + name + " patched!");
    ...
    public static boolean checkForCTMSupport() {
        ...
        try {
            ...
            Hardium.log.fine("CTM support found for water fix");
