...
import fr.mrpiano.hardium.blocks.WCBedBlock;
import fr.mrpiano.hardium.blocks.WCCakeBlock;
import fr.mrpiano.hardium.blocks.WCCropBlock;
import fr.mrpiano.hardium.blocks.WCCuboidBlock;
import fr.mrpiano.hardium.blocks.WCCuboidNEBlock;
import fr.mrpiano.hardium.blocks.WCCuboidNEStackBlock;
import fr.mrpiano.hardium.blocks.WCCuboidNSEWBlock;
import fr.mrpiano.hardium.blocks.WCCuboidNSEWStackBlock;
import fr.mrpiano.hardium.blocks.WCCuboidNSEWUDBlock;
import fr.mrpiano.hardium.blocks.WCDoorBlock;
import fr.mrpiano.hardium.blocks.WCFenceBlock;
import fr.mrpiano.hardium.blocks.WCFireBlock;
import fr.mrpiano.hardium.blocks.WCFurnaceBlock;
import fr.mrpiano.hardium.blocks.WCHalfDoorBlock;
import fr.mrpiano.hardium.blocks.WCLadderBlock;
import fr.mrpiano.hardium.blocks.WCLayerBlock;
import fr.mrpiano.hardium.blocks.WCLeavesBlock;
import fr.mrpiano.hardium.blocks.WCLogBlock;
import fr.mrpiano.hardium.blocks.WCPaneBlock;
import fr.mrpiano.hardium.blocks.WCPlantBlock;
import fr.mrpiano.hardium.blocks.WCRailBlock;
import fr.mrpiano.hardium.blocks.WCSandBlock;
import fr.mrpiano.hardium.blocks.WCSlabBlock;
import fr.mrpiano.hardium.blocks.WCSolidBlock;
import fr.mrpiano.hardium.blocks.WCSoulSandBlock;
import fr.mrpiano.hardium.blocks.WCSoundBlock;
import fr.mrpiano.hardium.blocks.WCStairBlock;
import fr.mrpiano.hardium.blocks.WCTorchBlock;
import fr.mrpiano.hardium.blocks.WCTrapDoorBlock;
import fr.mrpiano.hardium.blocks.WCWallBlock;
import fr.mrpiano.hardium.blocks.WCWebBlock;
...
public class HardiumBlockDef {
    ...
    public static class CustomColorMultHandler extends ColorMultHandler {
        ...
        protected void loadRes(String rname, String blkname) {
            try {
                ...
                Hardium.log.severe(String.format("Invalid color resource '%s' in block '%s'", rname, blkname));
    ...
    private static final Map<String, HardiumFactory> typeTable = new HashMap<String, HardiumFactory>();
    ...
    public Block[] createBlocks() {
        HardiumFactory bf = typeTable.get(blockType);
        if (bf == null) {
            Hardium.log.severe(String.format("Invalid blockType '%s' in block '%s'", blockType, blockName));
            return null;
        }
    ...
    public Material getMaterial() {
        ...
        if (m == null) {
            Hardium.log.warning(String.format("Invalid material '%s' in block '%s'", material, blockName));
    ...
    public Block.SoundType getStepSound() {
        ...
        if (ss == null) {
            Hardium.log.warning(String.format("Invalid step sound '%s' in block '%s'", stepSound, blockName));
    ...
    public CreativeTabs getCreativeTab() {
        ...
        if (ct == null) {
            Hardium.log.warning(String.format("Invalid tab name '%s' in block '%s'", creativeTab, blockName));
            ct = HardiumCreativeTab.tabWesterosBlocks;
        }
        return ct;
    }
    ...
    private void initMeta() {
        ...
        if (this.colorMultHandler == null) {
            Hardium.log.warning(String.format("Invalid colorMult '%s' in block '%s'", this.colorMult, blockName));
            ...
            for (Subblock sb : subBlocks) {
                ...
                if (sb.colorMult != null) {
                    ...
                    if (this.colorMultHandlerByMeta[sb.meta] == null) {
                        Hardium.log.warning(String.format("Invalid colorMult '%s' in block '%s'", sb.colorMult, blockName));
                ...
                if ((sb.particles != null) && (sb.particles.size() > 0)) {
                    ...
                    for (Particle pid : sb.particles) {
                        if (particles.contains(pid.particle) == false) {
                            Hardium.log.warning(String.format("Invalid particle '%s' in block '%s'", pid.particle, blockName));
    ...
    private static Block registerBlock(Block block, Class<? extends ItemBlock> itemclass, String name, int id)
    {
        ...
        {
            Hardium.crash(e, "Caught an exception during block registration");
            return null;
        }
    }
    ...
    public void doStandardRegisterIcons(IIconRegister ir) {
        ...
        if (subBlocks != null) {
            ...
            for (Subblock sb : subBlocks) {
                if (sb.textures == null) {
                    Hardium.log.warning(String.format("No textures for subblock '%d' of block '%s'", sb.meta, this.blockName));
    ...
    public EnumPlantType getPlantType(int meta) {
        ...
        if ((t != null) && (t.length() > 0)) {
            ...
            if (pt == null) {
                Hardium.log.severe(String.format("Invalid plant type '%s' at meta %d of block '%s'", t, meta, this.blockName));
    ...
    public boolean validateMetaValues(int[] valid_meta, int[] req_meta) {
        ...
        if (subBlocks != null) {
            for (Subblock sb : subBlocks) {
                ...
                if (!match) {
                    Hardium.log.severe(String.format("meta value %d for block '%s' is not valid for block type '%s'", sb.meta, this.blockName, this.blockType));
                    return false;
                }
                // If value exceeds metaMask-ed bits
                if ((m & metaMask) != m) {
                    Hardium.log.severe(String.format("meta value %d for block '%s' is not valid for block type '%s' - metaMask=%x", sb.meta, this.blockName, this.blockType, metaMask));
                    return false;
                }
            }
        }
        ...
        for (int req : req_meta) {
            ...
            if (!match) {
                Hardium.log.severe(String.format("Block '%s' is missing required meta value %d for block type '%s'", this.blockName, req, this.blockType));
                return false;
            }
        }
        return true;
    ...
    public static boolean sanityCheck(HardiumBlockDef[] defs) {
        HashSet<String> names = new HashSet<String>();
        BitSet metas = new BitSet();
        BitSet ids = new BitSet();
        // Make sure block IDs and names are unique
        for (HardiumBlockDef def : defs) {
            if (def == null) continue;
            if (def.blockName == null) {
                Hardium.log.severe("Block definition is missing blockName");
                return false;
            }
            if (names.add(def.blockName) == false) {    // If alreay defined
                Hardium.log.severe(String.format("Block '%s' - blockName duplicated", def.blockName));
                return false;
            }
            ...
            if (def.blockIDs != null) {
                for (int i = 0; i < def.blockIDs.length; i++) {
                    if (def.blockIDs[i] < 0) {  // Autoassign is OK
                        ...
                        Hardium.log.severe(String.format("Block '%s' - blockIDs[%d] invalid", def.blockName, i));
                        return false;
                    }
                    else if (ids.get(def.blockIDs[i])) {    // If already defined
                        Hardium.log.severe(String.format("Block '%s' - blockIDs[%d] duplicated", def.blockName, i));
                        return false;
                    }
                ...
                if (def.blockID < 0) {  // Autoassign is OK
                    ...
                    Hardium.log.severe(String.format("Block '%s' - blockID invalid", def.blockName));
                    return false;
                }
                else if (ids.get(def.blockID)) {    // If already defined
                    Hardium.log.severe(String.format("Block '%s' - blockID duplicated", def.blockName));
                    return false;
                }
                else {
                    ids.set(def.blockID);
            ...
            if (def.subBlocks != null) {
                for (Subblock sb : def.subBlocks) {
                    ...
                    if (metas.get(sb.meta)) {
                        Hardium.log.severe(String.format("Block '%s' - duplicate meta value %d", def.blockName, sb.meta));
                        return false;
                    }
                    metas.set(sb.meta);
                }
        ...
        Hardium.log.info("Hardium.json passed sanity check");
        return true;
    }
